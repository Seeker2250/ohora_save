...
<%@include file="header.jsp" %>
		...
		<div id="second-wrap-layout">
			...
			<div id="item-list-wrap">
								...
								<dl>
									...
									<div class="item-info">
										...
										<div class="cart-in">
											...
											<button onclick="deleteCookie('basketItems')">장바구니 쿠키 삭제</button>
				...
				$("div.cart-in img").on("click", function(event) {
					   let clickedPdtId = $(this).data("pdtid");  // 클릭한 상품의 ID
					   console.log("클릭 상품 ID:", clickedPdtId);
					   
					   // 장바구니 쿠키 확인
					   let basketItems = getCookie("basketItems");
					   let items = basketItems ? basketItems.split(",") : [];
					   console.log("현재 장바구니:", items);
					   
					   // 이미 담긴 상품인지 확인  
					   if(items.includes(String(clickedPdtId))) {
					       // 이미 있는 상품이면 alert
					   ...
					   // 새 상품이면
					   // 1. 쿠키 추가
					   items.push(String(clickedPdtId));
					   document.cookie = "basketItems=" + items.join(",") + "; path=/";
					   
					   // 2. 카운트 증가
					   let countElement = $(".count.EC-Layout-Basket-count");
	...
	<!-- <script>
	$("div.cart-in img").on("click", function(event) {
	    // 클릭한 상품의 ID 가져오기 - 실제 상품 ID인지 확인
	    let clickedPdtId = $(this).data("pdtid");
	    console.log("클릭한 상품:", clickedPdtId);
	    
	    // 이전 장바구니 상태 확인
	    let basketItems = getCookie("basketItems");
	    console.log("현재 장바구니 상태:", basketItems);

	    if(!basketItems) {
	        // 장바구니가 비어있을 때
	        document.cookie = "basketItems=" + clickedPdtId + "; path=/";
	        updateCartCount();
	        showModal();
	    } else {
	        // 이미 담긴 상품인지 체크
	        let items = basketItems.split(",");
	        if(items.includes(String(clickedPdtId))) {
	            alert("장바구니에 동일한 상품이 있습니다.\n장바구니에 추가하시겠습니까?");
	        } else {
	            // 새 상품 추가
	            items.push(clickedPdtId);
	            document.cookie = "basketItems=" + items.join(",") + "; path=/";
	            updateCartCount();
	            showModal();
	        }
